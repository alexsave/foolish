...
`docs/RUST_VS_C_KERNEL.md` is a full examination of whether the kernel should be rewritten in Rust, with `rust/` as its empirical half: four real kernel hot paths ported to safe Rust and measured against the shipped C over workloads generated by the real engine.
...
- **No rewrite is planned or started.**
  The doc is a decision record, not a work order, and `rust/` is a frozen snapshot of the measurement - it is not built by anything, not in CI, and will drift from `c/src` as the kernel changes.
