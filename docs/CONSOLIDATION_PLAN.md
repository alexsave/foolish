...
```bash
cd cnitro && make clean && make CC=clang -j4 all && make CC=clang tests   # 14 passed, 0 failed
...
1. **`cnitro/src/strategy.h`** — both sides claim ids 10-14/17. Take OUR
...
2. **`cnitro/Makefile`** — union. `CORE_SRC`: keep ours AND append their five
...
3. **`cnitro/src/main_eval.c`** — union: keep both dispatch case blocks (ours
...
> **`cnitro/src/cordite_sim.c` does NOT conflict** even though both the base
...
grep -R "novichok" supabase/functions/_shared/sdk/ts/wasm/bots.ts supabase/functions/_shared/common/bot_strategy.ts   # expect NO hits
```
