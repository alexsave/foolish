...
1. **Measure the real requirement** (`c/tests/l1_measure.c`, the stack
...
```sh
cd c && make build/rules.wasm build/guards.wasm
node -e "
const fs=require('fs');
...
`WASM_RULES_FLAGS` at `c/Makefile:310` instead of copying the above.)

Regenerate the visual layout any time: `bash c/tools/wasm_anatomy/generate.sh`
...
- `c/src/wasm_overlay.h` — offset macros + `_Static_assert` style.
- `c/src/replay.c` (search `CD_WASM_OVERLAY`) — how `g_rec`/`g_bn`
  become `#define`s over an anchor without touching native builds.
- `c/wasm/wasm_api.c` (search `CD_WASM_OVERLAY`) — same for
  `g_replay_io`/`g_io`.
...
1. Native: `cd c && make tests && ./build/cnitro_tests` (161/161) —
