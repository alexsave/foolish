...
> **Dated analysis, 2026-09-07.** The states and edges below were read from the code on that
> date and the anchors are `file:line`. The repo moved substantially on 2026-09-18 (the C game
> shape landed, `sdk/ts/gen` became a build artifact, the animation timing moved into the
> kernel), so treat the line numbers as a starting point and the code as the authority.
