...
- **CI never rebuilds the wasm** — it ships the committed `.gz`
  (`c/Makefile:264`). There is no "CI toolchain" to match.
...
```
cd c && make wasm-bots WASM_CC=/opt/homebrew/opt/llvm/bin/clang
```
...
Spec: `IOS_APP_DESIGN.md` §16.B4 (amended). Wire contract: `c/src/evwire.h`.
