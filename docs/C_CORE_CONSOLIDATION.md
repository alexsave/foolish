...
|---|---|---|---|
| F1 | **Bot roster: key → brain + tuning knobs + logs flag** | TS registry (`bot_strategy.ts:63-99`) · C table in `ios_api.c:37-48` **without knobs** → live strength/latency divergence (§3) | **DONE** (§4.1): `sdk/c/src/bot_roster.c` is the one table; the phone's knob + arena/prod drift is fixed. |
...
- **Native iOS bridge**: `sdk/c/ios/ios_api.c` (`fio_*`, JSON out) — the
...
**STATUS: LANDED (A1, July 2026)** — `sdk/c/src/bot_roster.{h,c}` +
`sdk/c/src/bot_knobs.{h,c}` exist; `ios_api.c` consumes them (§3.1 and the
arena/prod mix-up below are fixed); the `_max` tiers are gone; parity is pinned
by `e2e/bot_roster_parity.test.ts` + the roster tests in `sdk/c/tests/tests.c`.
...
`sdk/c/src/bot_drive.{h,c}` is the cycle; `fio_bot_drive_json` and
...
Verified as invariants in C (`sdk/c/tests/tests.c`), both mutation-checked:
