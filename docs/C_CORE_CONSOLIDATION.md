...
| A5 | **F4.2 replay steps from the kernel** — **KERNEL HALF DONE (§4.6)**: `replay_steps_v6` rebuilds the game from a v6 code (`start_game_with_deck`) and replays it through the real engine, so the events are `evwire_walk`'s, not a replay-side projection; `replay_decode_atoms_v6` + `ReplayHeader` are what the decoder had to say out loud (a round end is NOT recoverable from the log stream). `fio_replay_events_json` gives the phone the live event stream for a shared code. **v5 refused** — it hides the deal (owner: v5 is dead). | after A3 (reuses emitter), before native replay polish | **done (kernel)**: C invariants, both mutation-checked — rebuilt final board byte-identical to the played game's (np=2..6, unmasked) and a mid-game cut conserves the deck (this one caught a real missing-deck-tail bug that the finished-game test could not see); `ios-smoke` replays all 48 sweep games as live events, no hand leaked to a spectator. **Amended July 2026 — the web half is now KERNEL-COMPLETE and BLOCKED on the module, not on code.** `replay_steps_frames_v6` serializes a replay as the SAME packed evwire frames live play broadcasts (`wasm_replay_events` / `replayEventFrames`), chunked — `evwire_serialize` backpatches `n_events` as a u8, so one frame per game is impossible, and a whole game's frames (each carrying a masked board snapshot) outgrow any wasm IO buffer. `e2e/replay_steps_frames.test.ts` proves the frames decode with **`decodeEventWire` — the client's LIVE decoder — and come back as the game the engine played** (mutation-checked: a replay that skips pickups fails it). Each frame's trailer carries that step's board, which is exactly what `ReplayStep` is. **The module question, settled (July 2026).** `replay_steps.c` CANNOT link into rules.wasm — its decoded-action buffer alone is ~272 KB against rules.wasm's whole linear memory, PINNED at 196,608 B (`--initial-memory == --max-memory`), 1.4x the module before anything else. Shrinking the replay to fit was rejected (owner: one big module everywhere, split later — A10). So the CLIENT now loads bots.wasm: `ensureBotsAsync()` fetches `bots.wasm.gz` as a bundler asset (`new URL('./bots.wasm.gz', import.meta.url)`) and `__adoptEngine` puts the superset in the engine slot, so the browser runs the same kernel the server does. Deliberately a FETCHED ASSET, not a base64 embed like rules/guards: an embed is a second carrier of the same bytes — precisely how `rules_wasm.ts` went stale for weeks while bots.wasm.gz was rebuilt, leaving TWO kernels in the tree and making a C codec change invisible to `npm run test:validate` (it passed 39/39 against a kernel that did not have the change). `e2e/wasm_artifact.test.ts` pins that the shipped artifact exports what the client calls, since CI never rebuilds the wasm. **Still to do after the module lands:** rewire `ReplayScreen.tsx` + `Tutorial.tsx` onto the frames, delete `view.ts`'s v5 retrodiction (`slots`/`hidden`) and its oracle consumer `src/oracle/replayOracleInput.ts` (`nullSlots`), and note the scrubber needs REVERSE steps (`buildReverseSequences`), which the forward event stream does not provide — the per-frame trailer boards do. |
