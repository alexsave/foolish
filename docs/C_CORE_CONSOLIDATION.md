...
|---|---|---|---|
| F1 | **Bot roster: key → brain + tuning knobs + logs flag** | TS registry (`bot_strategy.ts:63-99`) · C table in `ios_api.c:37-48` **without knobs** → live strength/latency divergence (§3) | **DONE** (§4.1): `c/src/bot_roster.c` is the one table; the phone's knob + arena/prod drift is fixed. |
...
- **Native iOS bridge**: `c/ios/ios_api.c` (`fio_*`, JSON out) — the
...
**STATUS: LANDED (A1, July 2026)** — `c/src/bot_roster.{h,c}` +
`c/src/bot_knobs.{h,c}` exist; `ios_api.c` consumes them (§3.1 and the
arena/prod mix-up below are fixed); the `_max` tiers are gone; parity is pinned
by `e2e/bot_roster_parity.test.ts` + the roster tests in `c/tests/tests.c`.
...
`c/src/bot_drive.{h,c}` is the cycle; `fio_bot_drive_json` and
...
Verified as invariants in C (`c/tests/tests.c`), both mutation-checked:
...
**How it is built.** (Historical - `src/json_out.c` is deleted; see the A8 row.)
`src/json_out.c` — the emitter iOS already had, lifted out
...
- `wasm_unambiguous_cover` — **DONE (A7, July 2026).** The one-tap-cover
  affordance (was `coverCombinations.ts findUnambiguousCover`, 95 lines) is now
  `legal.c unambiguous_cover`, beside the `can_cover` it is built on. The web
  reaches it through `kernelUnambiguousCover` on the warm bots.wasm (A8
  KernelGate); all six call sites cut over and the TS file is deleted, so no
  cover logic lives in TS. +541 B gz, byte-reproducible. 8 mutation-checked C
  invariants + `e2e/unambiguous_cover.test.ts` (a fuzz with its own brute-force
  oracle: every accepted cover is legal and has exactly one achievable covered
  set). **The `fio/` wrapper for iOS/watch/iMessage is deferred** to when a
  native tap-commit surface is built — opportunistic, and the web was the first
  surface that needed it. The C resolver is ready for it.
...
| A7 | **F9 batch**: seed-len header + rejects — **`FOOLISH_SEED_LEN` DONE** (A4 needed it; `game.h`, and `game_set_deal_seed_bytes` rejects against it — the encode/decode-side rejects still ride the iMessage M0) —, `unambiguous_cover` (with the first surface that needs it), the `%`-name display tidy (**DROPPED, owner steer July 2026** — the `%` bot prefix is a data marker keeping replay codes minable for human-vs-bot moves; it is fine to SEE in games, do not strip it for display), and the **`console`+`gpt` drop** — delete the `CONSOLE` key (`types.ts:254`), the gpt lazy-load path + `strategies/gpt_strategy.ts`, and, once unreferenced, `move_stats.ts` + `pass_prob.ts` (neither strategy was ever seeded in production; `registerBotStrategy` itself STAYS — the offlinefun research harnesses use it) | opportunistic | per-item; grep proves no `console`/`gpt` strategy reference survives |
| A8 | **F7 wire-decode moves** — **SUPERSEDED.** This row shipped `src/json_out.c`: one JSON emitter in C, so the web could stop shadowing `view.c` and `evwire.c` with hand-written TypeScript. It is deleted. No JSON crosses any host boundary now (iOS never did after §16; the web read it through `wasm_view_json`/`wasm_events_json`, which are gone with it), and the web reads the two packed formats in `sdk/ts/wire/packed_read.ts`. That restores the duplication this row removed - the byte offsets are stated in C and in TypeScript again - and it is a deliberate trade, not an oversight: see that file's header. Cost recovered: 1,640 B gzipped off `bots.wasm` (70,667 → 69,027) and the emitter out of every module. |
| A9 | **Retire the TS parity twins for C invariants** (owner steer, July 2026) — once a surface is single-sourced in C, a TS re-implementation kept "byte-identical" by a parity test is no longer proving the port, it is *pinning the format in place*: it makes every kernel change a two-repo edit and it can only ever say "the copy agrees", never "the answer is right". Test INVARIANTS in C instead. **DONE (July 2026)**: every parity twin this row named is retired, each because a C invariant now covers it — not one deleted before its replacement existed. `e2e/replay_ts_oracle.ts` (1,078 lines) went first; `replay_codec.test.ts` asserts the decode reproduces the game the ENGINE played, strictly stronger than agreeing with a second codec. Then the two the row long called "remaining": **`e2e/replay_v6_parity.test.ts`** and its ~390-line v6 choreography (`collectV6`/`marshalInputV6`/`encodeReplayV6`) — deleted; `replay_v6.test.ts`'s two genuinely web-side survivors (the belief wire's DRAW masking, view.ts's resolved hands) now source their code from the PRODUCTION producer `kernelReplayEncodeV6FromGame`, so what they test is what ships, and their harness shed 45 lines of hand/stock reconstruction the marshalled encoder needed. **`e2e/bot_drive_parity.test.ts`** (406 lines) and its TS cycle — deleted; `processBotActionPacked` was reachable only from e2e (production `bot_actions.ts` calls `wasmBotDrive`), and the cycle's real invariants (fairness, bundling, determinism, pacing, snapshots, seeding) live in `c/tests/tests.c`, several negative-tested — 16 of them. A landmine worth keeping: `deriveTrump` is NOT choreography (the v5 encoder calls it); deleting it, only the tests caught it, because `tsc --noEmit` excludes `supabase` and `e2e` — for server/e2e code, running it IS the type check. **The evwire decode twin is GONE (A8, July 2026)**: `kernel_wire_decode.test.ts` was written as a mirror-vs-kernel harness for the cutover and, the moment the mirror was deleted, had nothing left to agree with — it now asserts the decode reproduces the game the ENGINE PLAYED, which is the strictly stronger claim. The cutover comparison is preserved in `bae4093`, which is where a cutover harness belongs once the cutover is made. **But `packed_wire_parity` does NOT die with A8, contrary to the A8 row's old expectation** — A8 killed the DECODE mirror; that test is about the ENCODER (`encodeEventWire`), which is still production for the lobby/meta path and cannot fold (the kernel only serializes streams it derived). Its hand-rolled byte scan is gone — it asks the kernel now, which is the honest way round: it inspects what a client could actually see, not what a second parser thinks is there. Each dies when its C invariant exists, NOT before | after the surface is single-sourced | the twin file is deleted and a C test fails when the behaviour breaks (mutation-check it) |
| A10 | **DONE (July 2026, branch `claude/a10-server-agnostic`)** — **Make the server code server-agnostic.** Final top-level shape: **`c/`** = the one C kernel (cnitro; the whole engine, not a binding); **`sdk/{ts,swift}`** = the per-language bindings to it (see `sdk/README.md`); **`server/`** = the backend, split into **`server/api/{core,common}`** (the host-neutral contract + game logic) and **`server/impls/supabase/`** (one concrete backend: the `adapter/` — DB, lease/CAS, realtime, `EdgeRuntime.waitUntil`, auth — plus the edge `functions/`, `config.toml`, `migrations/`, `seed.sql`). A second backend is a new sibling under `impls/`; `server/api/` never changes. The one-way DAG `core ← sdk ← common ← adapter` and the no-vendor-coupling rule (nothing above the adapter touches `jsr:@supabase`/`Deno.env`) are **enforced** by `e2e/validation/layering_validation.test.ts`. Deploy points at the impl: `supabase … --workdir server/impls`. **Verified locally:** web tsc 0 · validate 45/45 (incl. the layering invariants + the real server/lease/meta/concurrent suites over a live Postgres schema) · `make wasm-bots` byte-reproducible from `c/` · 3144 native C invariants · xcodegen + iOS arch-lint green. **Deploy-gated (needs one real run, on-branch until confirmed):** whether `deploy --use-api --workdir server/impls` bundles the cross-tree graph (edge fns import `../../../../api`, `../../../../../sdk/ts`) and honors the `../`-escaping `static_files` (fix-forward in `config.toml`); local `supabase start --workdir`; the iOS Xcode compile; the `coverage`/`ios`/`deploy` CI workflows. | done | a backend adapter can be swapped with no diff in `server/api` ✓ (DAG-enforced); each binding builds standalone against the wasm ✓ |
