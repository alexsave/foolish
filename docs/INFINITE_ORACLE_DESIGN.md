...
(`cnitro/src/cordite_sim.c:2350-2360`; `cordite_sim.h:209-213`). World #10⁶
...
is a bot-STRENGTH knob", `cnitro/Makefile:296-304`; collision knee ~TT10-11,
`docs/WASM_L1_BUDGET.md:177-197`). The server ships TT12+2WAY+PACK8 = 32 KiB
(one L1 page, `cnitro/Makefile:292`); native research default is TT16 = 1 MiB
...
all cordite scratch state are `_Thread_local`, `cnitro/Makefile:15-19`).
...
- The **trump-keep tax** (`OG_TRUMP_KEEP`, default 40 milli = 0.040 per trump
  ...
  (`cnitro/tools/og_explain/build_data.py:29,199-207`).
...
targets `og_explain` and `bots-wasm-explain`, `cnitro/Makefile:146-155,
...
- **Committed test codes**: 3-player `TUTORIAL_MOVES_CODE`
  ...
  `cnitro/tools/og_explain/samples/octogen-4v4.json` and `octogen-8way.json`
  (`url` fields).
...
| # | File | Change |
|---|------|--------|
| C1 | `cnitro/src/octogen_strategy.c` | `og_reload_flags()` — un-latch the env cache |
| C2 | `cnitro/wasm/wasm_bots_api.c` | `wasm_og_reload_flags()` export wrapper |
| C3 | `cnitro/src/octogen_strategy.c` (`og_ex_emit`) | emit verdict-only entries for solver-classified moves outside the candidate set |
...
Add to `cnitro/Makefile`, modeled directly on `bots-wasm-explain`
...
- Omitting the `-Wl,-z,stack-size=22528` override means the shared flags'
  ...
  `cnitro/Makefile:631-644`, `cordite_sim.c:1286-1302`), so this is pure
...
one new file `cnitro/wasm/wasm_oracle_mt.c` plus small `#ifdef` seams:
