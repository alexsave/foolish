...
| Auth is a username hash masquerading as email; no verification, no reset | `src/contexts/AuthContext.tsx:12-33`, `server/impls/supabase/config.toml` (`enable_confirmations = false`) | Rebuild required before payments (§4) |
| `cnitro` is freestanding C (clang, `-nostdlib` for wasm; plain C11 natively), thread-safety proven under the native OMP model (`_Thread_local` scratch/RNG/TT) | `c/Makefile`, design doc §3 | Compiles natively for iOS arm64 with pthreads — no wasm needed on iOS (§7) |
