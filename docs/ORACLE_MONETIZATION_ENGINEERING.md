...
| The same C engine already runs **on the server** (`bots.wasm` on Deno edge functions) | `supabase/functions/_shared/sdk/ts/wasm/` | Server-side verdicts are possible — but edge functions cap at **2s CPU** per invocation, so the server can issue tokens & spot-checks, not the "infinite" convergence |
