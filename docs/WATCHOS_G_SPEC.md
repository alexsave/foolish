...
- **Option H (all-vertical) WON — this spec is superseded in part.** The owner
  picked H; it is built and shipping in `ios/WatchFoolish`. Read
  `WATCHOS_LAYOUT.md` §4.6 + §4.6.1 (as-built) **first**, then use this file
  only for the parts H inherits unchanged: the rules facts (§0), the action
  decision table (§5 — H renders it as gray uppercase captions, not pills),
  the chooser (§5 G2b, minus the tiles behind the cards), the shield (§6),
  optimistic commit + reject glow (§7), haptics (§8), notifications (§9),
  non-play screens (§10), and the data layer.
  **Dead here:** §1's root vertical TabView and gesture map (H has no root
  pager and no horizontal gestures — the roster is pushed by a seat-strip
  tap), and §2's TablePager / FocusSlot / Pill / ChipStrip rows (replaced by
  the vertical table list + fisheye lane + caption).
