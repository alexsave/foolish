...
| Replay codec v5 (finished games → short URL) | `c/src/replay.{h,c}` (C is canonical; frozen wire format), TS bridge `supabase/functions/_shared/common/replay/` | The game-over artifact: a finished iMessage game becomes a normal `foolish.cards/<code>` replay, Oracle-analyzable |
| Packed action wire format | `supabase/functions/_shared/packed_action.ts`, `c/wasm/wire.h` | The per-move byte encoding the payload reuses verbatim |
