...
const strings_en: Record<StringId, string> = {
  ...
  oracle_why_repeat: '{clause} ({k}× in a row)',
  oracle_why_continues: ', and the game runs on (~{r} rounds in all)',
  oracle_why_nodata: 'No sampled alternatives for this decision — nothing to compare.',
  ...
  oracle_why_vs_pickup: 'Against {best} you would pick up {b}× per future; here it is {a}× — the biggest measured driver of the {delta} gap.',
  oracle_why_vs_trump: 'This line spends {a} trumps per future vs {b} after {best} — the biggest measured driver of the {delta} gap.',
  oracle_why_vs_opp: 'After {best} opponents are forced to pick up {a}× per future vs {b} here — the biggest measured driver of the {delta} gap.',
...
const strings_ru: Record<StringId, string> = {
  ...
  oracle_why_repeat: '{clause} ({k}× подряд)',
  oracle_why_continues: ', и игра продолжается (~{r} раундов всего)',
  oracle_why_nodata: 'Для этого решения нет выборки альтернатив — сравнивать нечего.',
  ...
  oracle_why_vs_pickup: 'После {best} вы забирали бы {b}× за партию; здесь {a}× — главный измеренный источник разницы в {delta}.',
  oracle_why_vs_trump: 'Эта линия тратит {a} козыря за партию против {b} после {best} — главный измеренный источник разницы в {delta}.',
  oracle_why_vs_opp: 'После {best} соперники вынуждены забирать {a}× за партию против {b} здесь — главный измеренный источник разницы в {delta}.',
...
const strings_ko: Record<StringId, string> = {
  ...
  oracle_why_repeat: '{clause} ({k}× 연속)',
  oracle_why_continues: ', 그리고 게임은 계속됩니다 (총 ~{r}라운드)',
  oracle_why_nodata: '이 결정에는 표본화된 대안이 없습니다 — 비교할 것이 없습니다.',
