...
const strings_en: Record<StringId, string> = {
  ...
  replay_title: 'Replay',
  invalid_replay: 'This replay link is invalid, or was recorded under an older version of the rules',
  ...
  oracle_ev_opp_beat: 'an opponent covers through',
  oracle_ev_opp_pickup: 'an opponent must pick up',
  oracle_ev_resolved: 'the game resolves right there',
  oracle_out_win: 'you escape and they are the fool',
  oracle_out_loss: 'you are the fool',
  oracle_out_mixed: 'you end up the fool {pct}% of the time',
  oracle_out_first: 'a clean first-out escape',
  oracle_out_strong: 'a strong finish (expected place {place})',
  oracle_out_mid: 'a middling finish (expected place {place})',
  oracle_out_bad: 'a poor finish (expected place {place})',
  oracle_why_repeat: '{clause} ({k} rounds running)',
  oracle_why_continues: ', and the game runs on (~{r} rounds in all)',
  oracle_why_nodata: 'No sampled futures for this move yet - nothing to prove from.',
  ...
  oracle_why_path: 'In {pct}% of futures: {chain} - {outcome}.',
  oracle_why_vs_pickup: 'Against {best} you would pick up {b} times per future; here it is {a} - the biggest measured driver of the {delta} gap.',
  oracle_why_vs_trump: 'This line spends {a} trumps per future against {b} after {best} - the biggest measured driver of the {delta} gap.',
  oracle_why_vs_opp: 'After {best} opponents are forced to pick up {a} times per future against {b} here - the biggest measured driver of the {delta} gap.',
  oracle_why_vs_diverge: 'The futures split from {best} at round {round}: here {here}, there {there} - worth {delta} expected finish.',
  oracle_why_vs_noise: 'No single cause dominates against {best}; many small differences add up to {delta}.',
  oracle_why_marginals: 'Over {n} futures: you pick up {mepk} times, they pick up {oppk}; you spend {metr} trumps, they spend {opptr}; ~{rnds} rounds to resolution.',
...
const strings_ru: Record<StringId, string> = {
  ...
  replay_title: 'Повтор',
  invalid_replay: 'Эта ссылка на повтор недействительна или записана по старой версии правил',
  is_the_fool: '- дурак',
  is_out: 'вышел',
  discarded: 'бито',
  draws: 'берёт',
  ...
  oracle_out_win: 'вы выходите, а в дураках соперник',
  oracle_out_loss: 'вы остаётесь дураком',
  oracle_out_mixed: 'вы остаётесь дураком в {pct}% случаев',
  oracle_out_first: 'чистый выход первым',
  oracle_out_strong: 'сильный результат (ожидаемое место {place})',
  oracle_out_mid: 'средний результат (ожидаемое место {place})',
  oracle_out_bad: 'слабый результат (ожидаемое место {place})',
  oracle_why_repeat: '{clause} ({k} раунда подряд)',
  oracle_why_continues: ', и игра продолжается (~{r} раундов всего)',
  oracle_why_nodata: 'Для этого хода ещё нет выборки вариантов - доказывать нечем.',
  ...
  oracle_why_path: 'В {pct}% вариантов: {chain} - {outcome}.',
  oracle_why_vs_pickup: 'После {best} вы забирали бы {b} раза за партию; здесь {a} - главный измеренный источник разницы в {delta}.',
  oracle_why_vs_trump: 'Эта линия тратит {a} козыря за партию против {b} после {best} - главный измеренный источник разницы в {delta}.',
  oracle_why_vs_opp: 'После {best} соперники вынуждены забирать {a} раза за партию против {b} здесь - главный измеренный источник разницы в {delta}.',
  ...
  oracle_why_marginals: 'За {n} партий: вы забираете {mepk} раза, они {oppk}; вы тратите {metr} козырей, они {opptr}; ~{rnds} раундов до развязки.',
...
const strings_ko: Record<StringId, string> = {
  ...
  replay_title: '리플레이',
  invalid_replay: '이 리플레이 링크가 잘못되었거나, 이전 버전의 규칙으로 기록되었습니다',
  is_the_fool: '바보입니다',
  is_out: '탈락',
  discarded: '버림',
  draws: '뽑기',
  ...
  oracle_out_first: '가장 먼저 깔끔하게 탈출',
  oracle_out_strong: '좋은 결과 (예상 순위 {place})',
  oracle_out_mid: '중간 결과 (예상 순위 {place})',
  oracle_out_bad: '나쁜 결과 (예상 순위 {place})',
  oracle_why_repeat: '{clause} ({k}라운드 연속)',
  oracle_why_continues: ', 그리고 게임은 계속됩니다 (총 ~{r}라운드)',
  oracle_why_nodata: '이 수에 대한 표본 미래가 아직 없습니다 - 증명할 근거가 없습니다.',
  oracle_why_best: '최선의 수 - 예상 순위 {ef}.',
  oracle_why_delta: '예상 순위 {ef} - {best}보다 {delta} 나쁨.',
  oracle_why_reply: '가장 유력한 응수: {phrase} (미래의 {pct}%).',
  oracle_why_then: ', 그다음 ',
  oracle_why_path: '미래의 {pct}%: {chain} - {outcome}.',
  oracle_why_vs_pickup: '{best} 이후에는 판당 {b}회 가져가지만 여기서는 {a}회 - {delta} 차이의 가장 큰 측정된 요인입니다.',
  oracle_why_vs_trump: '이 라인은 판당 트럼프를 {a}장 쓰지만 {best} 이후에는 {b}장 - {delta} 차이의 가장 큰 측정된 요인입니다.',
  oracle_why_vs_opp: '{best} 이후 상대는 판당 {a}회 가져가야 하지만 여기서는 {b}회 - {delta} 차이의 가장 큰 측정된 요인입니다.',
  oracle_why_vs_diverge: '{round}라운드에서 {best}와(과) 갈라집니다: 여기서는 {here}, 거기서는 {there} - 예상 순위 {delta}만큼의 값어치.',
