...
async function generateWoolPixelData(width: number, height: number): Promise<{
...
const generateWoolTextureAsync = async (width: number = 3840, height: number = 2160): Promise<string> => {
  ...
  if (!gl) {
    console.error('WebGL not available for wool texture, falling back to CPU-only');
    return generateWoolTextureFallback(width, height);
  }

  try {
    // Step 1: Generate pixel data using CPU (like fernFractal generates points)
    const pixelData = await generateWoolPixelData(width, height);
    ...
    console.error('WebGL wool texture generation failed, using fallback:', error);
    return generateWoolTextureFallback(width, height);
  }
};
