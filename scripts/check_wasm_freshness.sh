...
ARTIFACTS=(
  ...
  # sdk/ts/gen/{game_layout,layout_hash}.bots.ts used to be listed here. They are
  # not artifacts any more: they are generated by every build that reads them
  # (tools/structgen/gen.sh) and committed nowhere, so there is no copy of them
  # that can be behind anything. What still has to hold is that the committed
  # WASM keeps the layout they describe, and that is checked where it matters -
  # at load, by LAYOUT_HASH (e2e/layout_hash.test.ts).
)
