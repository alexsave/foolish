#!/usr/bin/env bash
# CI: install the PINNED wasm toolchain and build the three shipped modules
# (scripts/wasm_build.sh). Sourced or run by every lane that needs them.
#
...
#
# Usage:
#   . scripts/ci_wasm.sh      # install, export WASM_CC/LLVM_PREFIX/PATH, build
#   bash scripts/ci_wasm.sh   # same, but the exports die with the subshell
set -euo pipefail
...
echo "--- the toolchain the shipped modules are built with ---"
...
echo "--- what shipped-module build produced ---"
