...
export interface GameLog extends UnsavedGameLog {
    ...
    // Insert-order sequence, assigned by the database. Only present on rows
    // loaded back from game_logs; used with created_at as a total order for the
    // replay encoder (created_at alone ties within a move's cascade).
    seq?: number;
}