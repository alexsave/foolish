...
import {
    ...
    game_done,
    other_player,
} from '../common/common_utils.ts';
import { Card, Game, GAME_STATUS, PLAYER_STATUS, PersonalGame, PrivatePlayer, PublicGame, PlayerHand, UserEloRating, BotHand, AnimationEvent, PublicAnimationEvent, PersonalAnimationEvent, ANIMATION_EVENT_TYPE, GameLog, LOG_TYPE, STRATEGY_KEY } from '../core/types.ts';
...
import type { EncodedReplay } from '../common/replay/core.ts';
...
export const loadCompleteGame = async (game_id: string): Promise<Game> => {
    ...
    if (data.state && data.status !== GAME_STATUS.WAITING) {
        ...
        const { hexToBytes } = await import('../common/replay/codec.ts');
...
export const commitGame = async (
    ...
    if (p_state === null && (game.status === GAME_STATUS.PLAYING || game.status === GAME_STATUS.GAME_OVER)) {
        ...
        const { bytesToHex } = await import('../common/replay/codec.ts');
    ...
    try {
        const { buildPlayerViewRows, buildSpectatorView } = await import('../common/player_views.ts');
...
export const finalizeEndedGame = async (game: Game): Promise<void> => {
    ...
    try {
        ...
        const { verifyRoundTripV6FromGame } = await import('../common/replay/encode.ts');
        const { encodeExtras, moveTimesFromLogs } = await import('../common/replay/extras.ts');
        const { base32Decode, bytesToHex, hexToBytes } = await import('../common/replay/codec.ts');
        ...
        // `extras` the names + timing blob (_shared/common/replay/extras.ts). The
