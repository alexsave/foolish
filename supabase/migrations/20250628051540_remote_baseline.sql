-- Baseline: the pre-migration schema (backfilled).
--
-- The hosted project (wngpfwmwkltonwosqflx) records version 20250628051540 in
-- supabase_migrations.schema_migrations as its initial baseline. Tracked
-- migrations only began in earnest in June 2026; before that the schema was
-- maintained by pasting supabase/seed.sql into the SQL editor. This file was an
-- EMPTY placeholder, which made a from-scratch `supabase db reset` fail — the
-- later `ALTER TABLE games ...` migrations had no tables to alter
-- ("relation \"games\" does not exist").
--
-- Backfill = supabase/seed.sql exactly as it stood at commit 0134ac2 (the parent
-- of the first tracked migration, 20260615 reserve_bot_username_prefix). That is
-- precisely the state the 27 subsequent migrations were authored against, so
-- they now apply cleanly on top. Local `db reset` then finishes by running the
-- CURRENT seed.sql, which DROP ... CASCADEs and rebuilds the final schema.
--
-- REMOTE IS UNAFFECTED: version 20250628051540 is already applied there, so
-- `supabase db push` never re-runs this body. It only ever executes on a
-- from-scratch local database.
-- =============================================================================
