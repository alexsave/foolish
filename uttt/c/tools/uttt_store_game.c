 ...
 *
 * The first 25 plies are fixed: the rig's 18-ply `devgame` opening and the
 * seven moves the "Send your moves" frame was played with (ply 25 is the
 * staged draft). The rest is fountain for both seats. Every ply goes through
 * uttt_play, the whole list is replayed from scratch through the rules again,
 * and the replay link (drawing seed 77, the DEBUG seeded game's) is read back
 * and compared. See docs/STORE_SHOTS.md.
 *
 *   make store-game          (seed 1, 2000 rollouts - the recorded game) */
#include <stdio.h>
#include <stdlib.h>
#include "../src/uttt.h"
