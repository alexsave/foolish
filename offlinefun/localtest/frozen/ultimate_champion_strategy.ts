import { Card, Game, PrivatePlayer } from '../../../supabase/functions/_shared/core/types.ts';
...
export class UltimateChampionStrategy implements BotStrategy {
    ...
    async chooseMove(game: Game, botPlayerId: string, legalMoves: LegalMove[]): Promise<LegalMove> {
        ...
        if (waitMoves.length > 0) {
            console.log(`Bot ${bot.name} (ultimate_champion) chooses to wait - other players still attacking`);
            return waitMoves[0];
        }
