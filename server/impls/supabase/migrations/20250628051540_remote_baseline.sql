-- Baseline: the schema the tracked migrations were written against.
--
-- The hosted project (wngpfwmwkltonwosqflx) records version 20250628051540 in
-- supabase_migrations.schema_migrations as its initial baseline. Before June 2026
-- the schema was kept by pasting seed.sql into the SQL editor, so no migration
-- created the tables, and this file used to be an empty placeholder. A
-- from-scratch `supabase start` or `supabase db reset` then failed at the first
-- later migration that alters a table: `relation "games" does not exist`.
--
-- The body is seed.sql exactly as it stood at 0134ac2b, the parent of the first
-- tracked migration (20260615120000_reserve_bot_username_prefix), which is the
-- state every later migration was authored against (two em dashes in comments
-- are hyphens). A local start applies it, then every migration, then the current
-- seed.sql, which drops and rebuilds the final schema.
--
-- HOSTED IS UNAFFECTED: 20250628051540 is already recorded as applied there, so
-- `supabase db push` never runs this body. e2e/db_migration_grants.test.ts does not
-- run it either: it starts from a frozen hosted schema and replays the migrations
-- from 20260807120000 on.
-- =============================================================================
...
-- (Guarded, the one change from 0134ac2b besides the dashes: a local Supabase
-- image makes supabase_realtime_admin the owner of realtime.messages, where RLS
-- is already on, and `postgres` may not ALTER it.)
DO $$ BEGIN
