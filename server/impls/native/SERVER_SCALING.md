...
| `og_ex_*` (octogen's deliberation-dump buffers) | `octogen_strategy.c` | **No — compiled out** | Behind `#ifdef OG_EXPLAIN_BUILD`, defined only by `c/Makefile`'s standalone analysis targets (`wasm-oracle`, `wasm-oracle-mt`, `bots-wasm-explain`), never by either server Makefile. |
