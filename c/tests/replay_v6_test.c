...
static void run_one(int np, int strat, uint32_t seed) {
    ...
    CHECK(g_dec[0] == REPLAY_FORMAT_VERSION_V6, "version %d", g_dec[0]);
    ...
    if (atoms >= 4) {
        ...
        CHECK(g_dec[0] == REPLAY_FORMAT_VERSION_V6, "mid version");
...
static void run_one_from_game(int np, int strat, uint32_t seed) {
    ...
    if (atoms > 4) {
        ...
        CHECK(g_dec[0] == REPLAY_FORMAT_VERSION_V6, "mid version %d", g_dec[0]);
