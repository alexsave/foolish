...
//     c/tools/endgame_retro/encode_replay.mjs moves.json
...
const { start_game_packed, reconstructSeededDeal } = await import(`${R}/server/api/common/game_lifecycle.ts`);
...
const { verifyRoundTripV6 } = await import(`${R}/server/api/common/replay/encode.ts`);
const { PLAYER_STATUS, GAME_STATUS, STRATEGY_KEY } = await import(`${R}/server/api/core/types.ts`);
