...
| `narrow_path.c` | Self-play across a seed range; for the seat that WINS, measure the longest run of *consecutive* own endgame decisions (with a real choice) where retrograde says exactly ONE move wins — a forced tightrope to victory. Reports seeds with `forced_run >= MINRUN`. |
| `trace_path.c` | Trace one seed: print every deck-empty endgame decision with its full per-move WIN/LOSS breakdown and octogen's pick, marking the `<<forced` unique-win nodes. Use it to read out a narrow path move-by-move. |
