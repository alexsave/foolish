...
console.log(JSON.stringify({
  ...
  modules: [
    ...
    { key:'bots', human:'bots.wasm', wasm:`${B}/bots.wasm`, named:`${B}/named/bots.named.wasm`,
      blurb:'The rules kernel PLUS every algorithmic bot strategy and the choose-move bridge. A superset of rules.wasm, loaded only where bots run. Ships as a gzip static asset, not a base64 embed. Linear memory: 18 pages (1.13 MiB) initial, down from 64; it GROWS on first play to 24 pages (~1.5 MiB peak, what CI/edge charge) when the cordite solver bump-allocates its transposition table — shrunk 1 MiB -> 256 KiB (CD_TT_BITS 16->13) after a per-game move-divergence study showed all shipped solver bots bit-identical to a collision-free table at that size (octogen is the binding bot, cliff at TT7). Down from a 5.13 MiB base peak (-71%). Fitting bots in L1 is a solver problem, not a layout one.' },
  ],
}));
NODE
