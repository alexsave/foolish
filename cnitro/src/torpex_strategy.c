// Torpex — semtex + a LEARNED VALUE NET replacing rollouts (TX_VALUE=1,
// default when weights load). The determinized MC machinery is semtex's
// verbatim; each (world x candidate) is evaluated by V(state after the
// root move) — an MLP trained on semtex self-play outcomes (main_gen.c +
// train_torpex.py + torpex_value.c) — instead of a handwritten-policy
// playout. See TORPEX.md. Derived from semtex_strategy.c.
//
...
static inline int tx_card_score(Card c, int power) {
    return c.value + (c.suit == power ? 1000 : 0);
}

static bool tx_set_contains(const Card *arr, int n, Card c) {
    for (int i = 0; i < n; i++) if (card_eq(arr[i], c)) return true;
    return false;
}

static uint32_t tx_xorshift(uint32_t s) {
    s ^= s << 13; s ^= s >> 17; s ^= s << 5;
    return s ? s : 0xB1A570u;
}

static uint32_t tx_mix(uint32_t a, uint32_t b) {
    uint32_t h = a * 0x9E3779B1u ^ (b + 0x7F4A7C15u);
    h ^= h >> 16; h *= 0x85EBCA77u; h ^= h >> 13;
    return h ? h : 1;
}

// Ablation switches (read once): TX_NO_SOLVE / TX_NO_VOIDS / TX_NO_FLIP /
// TX_NO_FLOORS / TX_NO_LEAF / TX_NO_AVOID / TX_VERIFY, plus TX_W1/TX_W2
// world-count overrides for tuning.
static int tx_flag(const char *name) {
    const char *v = getenv(name);
    return v && v[0] && v[0] != '0';
}
static int tx_env_int(const char *name, int def) {
    const char *v = getenv(name);
    return (v && v[0]) ? atoi(v) : def;
}
static _Thread_local int tx_flags_loaded = 0;
static _Thread_local int tx_no_solve = 0, tx_no_voids = 0, tx_no_flip = 0;
static _Thread_local int tx_no_floors = 0, tx_no_leaf = 0, tx_no_avoid = 0;
static _Thread_local int tx_no_earlyexit = 0;
static _Thread_local int tx_verify = 0;
static _Thread_local int tx_no_fastroll = 0;   // TX_NO_FASTROLL=1: struct rollout
...
// TX_BBLEAF: 2 (default) = pc-aware exact leaf endgames in rollouts —
// small (TX_BBLEAF_CARDS2, 8) leaves heads-up only; TX_BBLEAF_CARDS
...
// wall-clock; the replicated win is the heads-up leaf. 1 = TX_BBLEAF_CARDS
// everywhere; 0 = off.
static _Thread_local int tx_bbleaf = 2;
static _Thread_local int tx_bbleaf_cards2 = 8;
// TX_ADAPT: void-contradiction => per-seat distrust of floors+voids (on by
// default — pure evidence, no downside). TX_PROFILE: weak-seat detection +
// LOOSE rollout model for profiled seats.
static _Thread_local int tx_adapt = 1;
// TX_VALUE: 1 (default) = evaluate (world x candidate) with the learned
// value net when weights are available; 0 = semtex's rollout playouts
// (bit-identical to semtex). Requires $TORPEX_WEIGHTS.
static _Thread_local int tx_value_on = 1;
...
static _Thread_local int tx_void_mod = 4;
static _Thread_local int tx_profile = 0;
...
static _Thread_local int tx_solve_cards = 20;
static _Thread_local int tx_bbleaf_cards = 12;
static _Thread_local long tx_bbleaf_budget = 3000;
// Per-seat rollout policy map for the current decision (NULL = all
// handwritten). Set by torpex_strategy_choose when profiling flags seats.
static _Thread_local uint8_t tx_polmap_buf[MAX_PLAYERS];
static _Thread_local const uint8_t *tx_polmap = NULL;
static _Thread_local int tx_bbleaf_on = 0;   // effective flag for this decision
static _Thread_local int tx_bbleaf_cards_eff = 12;
static _Thread_local int tx_difftest = 0;      // TX_DIFFTEST=1: assert fast==slow
static _Thread_local int tx_w1_override = 0, tx_w2_override = 0;
static _Thread_local int tx_w3_override = -1;
static _Thread_local int tx_old_budget = 0;    // cordite_old variant: pre-2x worlds
static _Thread_local int tx_keep1 = 0, tx_keep2 = 0;  // TX_KEEP1/2: candidates kept past stage 0/1 (0=default n/3, 2)
// Oracle mode (research): multiply the world budget and widen candidate
// survival for one call. Used by torpex_oracle_strategy_choose to audit
// loss games — a decision the oracle changes was compute-limited.
static _Thread_local int tx_oracle = 0;
static _Thread_local int tx_rollout_policy = 0;       // TX_ROLLOUT: 0=default, 1=espresso, 2=handwritten (struct path)
...
// in less wall-clock. Tunable via TX_BB_WIN / TX_BB_AVOID for sweeps.
static _Thread_local int tx_bb_win_budget = 20000;
static _Thread_local int tx_bb_avoid_budget = 15000;

static int tx_in_count(const Game *g) {
    int n = 0;
...
static bool tx_apply(Game *g, int p_idx, const LegalMove *m) {
    switch (m->type) {
...
static void tx_lite_clone(Game *dst, const Game *src) {
...
typedef struct {
    ...
    Card voids[MAX_PLAYERS][TX_MAX_VOIDS];
...
static bool tx_void_forbidden(const Belief *B, const Game *g, int p, Card c) {
...
static bool tx_floor_forbidden(const Belief *B, const Game *g, int p, Card c) {
    return B->floor_v[p] > 0 && c.suit != g->power_suit && c.value < B->floor_v[p];
}

static void tx_pinned_remove(Belief *B, int p, Card c) {
...
static void tx_pinned_add(Belief *B, int p, Card c) {
    if (B->pinned_n[p] >= MAX_HAND_SIZE) return;
    if (tx_set_contains(B->pinned[p], B->pinned_n[p], c)) return;
...
static void tx_floor_check(Belief *B, const Game *g, int p, Card c) {
...
static void tx_build_belief(const Game *g, int bot_idx, Belief *B) {
    memset(B, 0, sizeof(*B));
    ...
    int flip_log_idx = (!deck_alive_now && !tx_no_flip) ? last_draw_idx : -1;
        ...
        switch (L->log_type) {
            ...
            case LOG_PASS: {
                ...
                for (int k = 0; k < L->num_pairs; k++) {
                    ...
                    if (p >= 0 && p != bot_idx) {
                        ...
                        if (!tx_set_contains(B->pinned[p], B->pinned_n[p], c)
                            && tx_void_forbidden(B, g, p, c)) B->mc_tell[p] = true;
            ...
            case LOG_PICKUP:
                ...
                if (p >= 0 && p != bot_idx) {
                    ...
                    if (unc_n == 1 && B->void_n[p] < TX_MAX_VOIDS) {
                        B->voids[p][B->void_n[p]++] = unc[0];
                    }
                    for (int k = 0; k < tbl_n; k++) tx_pinned_add(B, p, tbl[k]);
            ...
            case LOG_DRAW:
                ...
                if (p >= 0 && p != bot_idx) {
                    ...
                    B->floor_v[p] = 0;
                    if (i == flip_log_idx) tx_pinned_add(B, p, g->flipped);
                }
                break;
            case LOG_PLAYER_OUT:
                in_now--;
    ...
    for (int suit = 0; suit < 4; suit++) {
        for (int v = start_v; v <= ACE_VALUE; v++) {
            ...
            if (!tx_set_contains(known, kn, c)) B->pool[B->n++] = c;
        }
    }
    ...
    for (int p = 0; p < g->num_players; p++) {
        ...
        int allowed = 0;
        for (int i = 0; i < B->n; i++) {
            if (!tx_void_forbidden(B, g, p, B->pool[i])
                && !tx_floor_forbidden(B, g, p, B->pool[i])) allowed++;
        }
        if (allowed < unknown && B->floor_v[p] > 0) {
            B->floor_v[p] = 0;
            allowed = 0;
            for (int i = 0; i < B->n; i++) {
                if (!tx_void_forbidden(B, g, p, B->pool[i])) allowed++;
            }
        }
...
static _Thread_local int tx_full_logs = 0;   // TX_FULL_LOGS=1: bp-style worlds

static void tx_sample_world(Game *g_out, const Game *g_in, int my_idx,
    ...
    for (int si = 0; si < ns; si++) {
        ...
        bool bad = (use_v && tx_void_forbidden(B, g_in, p, c))
                || (use_f && tx_floor_forbidden(B, g_in, p, c));
        if (!bad) continue;
        for (int d = 0; d < deck_n; d++) {
            Card dc = g_out->deck[d];
            bool dc_bad = (use_v && tx_void_forbidden(B, g_in, p, dc))
                       || (use_f && tx_floor_forbidden(B, g_in, p, dc));
            if (!dc_bad) {
                g_out->deck[d] = c;
...
#define TX_SOLVE_MAX_DEPTH   48
#define TX_SOLVE_MAX_MOVES   96
#define TX_SOLVE_BUDGET      200000L
#define TX_AVOID_BUDGET      150000L
#define TX_SOLVE_MAX_CARDS   20
#define TX_LEAF_BUDGET       1500L

typedef struct {
    long budget;
    bool aborted;
    int  me;
    Game       *child;   // [TX_SOLVE_MAX_DEPTH]
    LegalMoves *mv;      // [TX_SOLVE_MAX_DEPTH]
} Solver;

static _Thread_local Game       *tx_solver_child = NULL;
static _Thread_local LegalMoves *tx_solver_mv = NULL;

static bool tx_solver_ready(void) {
    if (!tx_solver_child) {
        tx_solver_child = malloc(sizeof(Game) * TX_SOLVE_MAX_DEPTH);
        tx_solver_mv    = malloc(sizeof(LegalMoves) * TX_SOLVE_MAX_DEPTH);
    }
    return tx_solver_child && tx_solver_mv;
}
...
static int tx_solve(Solver *S, const Game *g, int alpha, int beta, int depth) {
    int loser = game_done(g);
    if (loser >= 0) return (loser == S->me) ? -(1000 - depth) : (1000 - depth);
    if (tx_in_count(g) == 0) return 0;   // defensive: simultaneous out
    if (depth >= TX_SOLVE_MAX_DEPTH) { S->aborted = true; return 0; }
    ...
    if (mv->n == 0) return 0;
    if (mv->n > TX_SOLVE_MAX_MOVES) { S->aborted = true; return 0; }

    bool maximizing = (actor == S->me);
    int best = maximizing ? -2000 : 2000;
    for (int i = 0; i < mv->n; i++) {
        Game *child = &S->child[depth];
        tx_lite_clone(child, g);
        if (!tx_apply(child, actor, &mv->moves[i])) continue;
        int v = tx_solve(S, child, alpha, beta, depth + 1);
...
static _Thread_local long tx_leaf_budget = TX_LEAF_BUDGET;
static _Thread_local int  tx_leaf_max_cards = 0;   // set from env at init
static _Thread_local int  tx_floor_mod = 2;        // floors in 1/mod worlds

static int tx_leaf_solve(const Game *g) {
    if (!tx_solver_ready()) return -1;
    int me = -1;
    for (int i = 0; i < g->num_players; i++) {
    ...
    Game root;
    tx_lite_clone(&root, g);
    root.num_logs = 0;   // solver never reads history

    Solver S;
    S.budget  = tx_leaf_budget;
    S.aborted = false;
    S.me      = me;
    S.child   = tx_solver_child;
    S.mv      = tx_solver_mv;
...
static StrategyFn tx_rollout_for(const Game *g) {
    // TX_ROLLOUT (struct path only): 1 = espresso everywhere, 2 = handwritten
    ...
    // that bias. Run with TX_NO_FASTROLL=1.
    if (tx_rollout_policy == 1) return espresso_strategy_choose;
    if (tx_rollout_policy == 2) return handwritten_strategy_choose;
    bool deck_active = (g->deck_count > 0 || g->has_flipped);
    if (deck_active || tx_in_count(g) == 2) return handwritten_strategy_choose;
...
static int tx_simulate(Game *g, int my_idx, int max_turns) {
    int turns = 0;
    ...
    while (game_done(g) < 0 && turns++ < max_turns) {
        ...
        if (!tx_no_leaf && !leaf_tried && g->deck_count == 0 && !g->has_flipped
            && tx_in_count(g) == 2) {
            int total = 0;
            for (int i = 0; i < g->num_players; i++) {
            ...
            if (total <= tx_leaf_max_cards) {
                leaf_tried = true;
                int loser = tx_leaf_solve(g);
                if (loser >= 0) {
        ...
        for (int pi = 0; pi < g->num_players; pi++) {
            ...
            StrategyFn fn = tx_rollout_for(g);
            int idx = fn(g, pi, &moves, NULL);
            if (idx < 0 || idx >= moves.n) continue;
            if (tx_apply(g, pi, &moves.moves[idx])) { acted = true; break; }
        }
        if (!acted) break;
    }
...
// and play it out on bitmasks. ~10x faster per ply than tx_simulate. The
// effective rollout policy is always handwritten (see cordite_sim.c), and the
// exact leaf solver (TX_LEAF, off by default) is not used in the fast path.
static int tx_simulate_fast(const Game *g, int my_idx, int max_turns) {
    SimState s;
    cd_sim_from_game(&s, g);
    if (tx_bbleaf_on || tx_polmap)
        return cd_sim_playout_pol(&s, my_idx, max_turns, !tx_no_earlyexit,
                                  tx_bbleaf_on ? tx_bbleaf_cards_eff : 0,
                                  tx_bbleaf_budget, tx_polmap);
    return cd_sim_playout(&s, my_idx, max_turns, !tx_no_earlyexit);
}

// Dispatcher: bitboard rollout by default; struct rollout under TX_NO_FASTROLL
// or TX_LEAF (the leaf solver lives on the struct path). TX_DIFFTEST runs both
// and tallies divergences (printed at process exit by the eval harness if it
// hooks tx_difftest_report, else just counted).
static _Thread_local long tx_diff_total = 0, tx_diff_mismatch = 0;
static int tx_rollout(Game *g, int my_idx, int max_turns) {
    if (tx_no_fastroll || !tx_no_leaf) return tx_simulate(g, my_idx, max_turns);
    if (tx_difftest) {
        uint32_t rng0 = game_rng_get();
        int fast = tx_simulate_fast(g, my_idx, max_turns);
        game_rng_set(rng0);
        Game slow_g;
        tx_lite_clone(&slow_g, g);
        int slow = tx_simulate(&slow_g, my_idx, max_turns);
        tx_diff_total++;
        if (fast != slow) {
            tx_diff_mismatch++;
            if (tx_diff_mismatch <= 20) {
                fprintf(stderr, "TX_DIFFTEST mismatch #%ld: fast=%d slow=%d "
                        "(np=%d deck=%d logs=%d)\n", tx_diff_mismatch, fast, slow,
                        g->num_players, g->deck_count, g->num_logs);
            }
        }
        return slow;  // keep slow behavior while difftesting
    }
    return tx_simulate_fast(g, my_idx, max_turns);
}

void tx_difftest_report(void) {
    if (tx_diff_total > 0) {
        fprintf(stderr, "TX_DIFFTEST: %ld/%ld rollouts diverged (%.3f%%)\n",
                tx_diff_mismatch, tx_diff_total,
                100.0 * (double)tx_diff_mismatch / (double)tx_diff_total);
    }
}
...
static int tx_try_endgame_solve(const Game *g, int bot_idx,
    ...
    *n_safe = moves->n;
    ...
    if (total > tx_solve_cards) return -1;
    ...
    // TX_NO_BBSOLVE=1 falls back to the struct solver for A/B.
    bool bbsolve = !tx_flag("TX_NO_BBSOLVE");
    SimState root_sim;
    if (bbsolve) cd_sim_from_game(&root_sim, &root);
    cd_sim_solve_reset();

    Solver S;
    S.budget  = TX_SOLVE_BUDGET;
    S.aborted = false;
    S.me      = bot_idx;
    S.child   = tx_solver_child;
    S.mv      = tx_solver_mv;
    long win_budget   = bbsolve ? (long)tx_bb_win_budget   : TX_SOLVE_BUDGET;
    long avoid_budget = bbsolve ? (long)tx_bb_avoid_budget : TX_AVOID_BUDGET;
    ...
    for (int i = 0; i < moves->n; i++) {
        ...
        if (bbsolve) {
            ...
            Game child;
            tx_lite_clone(&child, &root);
            if (!tx_apply(&child, bot_idx, &moves->moves[i])) continue;
            S.aborted = false;
            v = tx_solve(&S, &child, alpha, 2000, 1);
            aborted_i = S.aborted;
...
#define TX_MAX_CANDS 26

typedef struct {
    int idx[TX_MAX_CANDS];
    int n;
} Candidates;

static void tx_ranked_insert(int *idxs, double *keys, int *n, int cap,
                             int idx, double key) {
    int pos = *n;
...
static void tx_pick_candidates(const Game *g, const LegalMoves *moves,
    ...
    for (int i = 0; i < moves->n; i++) {
        ...
        switch (m->type) {
            case MOVE_ATTACK: {
                int sum = 0;
                for (int j = 0; j < m->n_cards; j++) sum += tx_card_score(m->cards[j], power);
                tx_ranked_insert(atk, atk_k, &n_atk, 12, i,
                                 -(double)m->n_cards * 10000.0 + (double)sum);
                break;
            }
            case MOVE_COVER: {
                double prod = 1.0;
                for (int j = 0; j < m->n_cards; j++) prod *= (double)tx_card_score(m->cards[j], power);
                tx_ranked_insert(cov, cov_k, &n_cov, 10, i,
            ...
            case MOVE_PASS: {
                int sum = 0;
                for (int j = 0; j < m->n_cards; j++) sum += tx_card_score(m->cards[j], power);
                tx_ranked_insert(pas, pas_k, &n_pas, 3, i, (double)sum);
                break;
            }
    ...
    out->n = 0;
    for (int i = 0; i < n_atk && out->n < TX_MAX_CANDS; i++) out->idx[out->n++] = atk[i];
    for (int i = 0; i < n_cov && out->n < TX_MAX_CANDS; i++) out->idx[out->n++] = cov[i];
    for (int i = 0; i < n_pas && out->n < TX_MAX_CANDS; i++) out->idx[out->n++] = pas[i];
    if (good_idx >= 0 && out->n < TX_MAX_CANDS)   out->idx[out->n++] = good_idx;
    if (pickup_idx >= 0 && out->n < TX_MAX_CANDS) out->idx[out->n++] = pickup_idx;
}
...
static void tx_params(int num_players, int *W1, int *W2, int *W3) {
    ...
    if (tx_oracle) { *W1 *= 6; *W2 *= 6; *W3 *= 6; }
    if (tx_w1_override > 0) *W1 = tx_w1_override;
    if (tx_w2_override > 0) *W2 = tx_w2_override;
    if (tx_w3_override >= 0) *W3 = tx_w3_override;
}

// TX_VERIFY=1: oracle self-check (test-only — reads real hands to validate
// the public-info belief, never to play).
static void tx_verify_belief(const Game *g, int bot_idx, const Belief *B) {
    for (int p = 0; p < g->num_players; p++) {
        if (p == bot_idx) continue;
        ...
        for (int k = 0; k < B->pinned_n[p]; k++) {
            ...
            if (!found) {
                fprintf(stderr, "TX_VERIFY: pinned card v%d s%d NOT in p%d hand (logs=%d)\n",
                        B->pinned[p][k].value, B->pinned[p][k].suit, p, g->num_logs);
            }
        }
        for (int j = 0; j < pl->hand_count; j++) {
            if (tx_set_contains(B->pinned[p], B->pinned_n[p], pl->hand[j])) continue;
            if (B->void_n[p] > 0 && tx_void_forbidden(B, g, p, pl->hand[j])) {
                fprintf(stderr, "TX_VERIFY: void violated: p%d holds v%d s%d\n",
                        p, pl->hand[j].value, pl->hand[j].suit);
            }
            if (tx_floor_forbidden(B, g, p, pl->hand[j])) {
                fprintf(stderr, "TX_VERIFY: floor violated: p%d holds v%d s%d floor=%d\n",
                        p, pl->hand[j].value, pl->hand[j].suit, B->floor_v[p]);
            }
            if (!tx_set_contains(B->pool, B->n, pl->hand[j])) {
                fprintf(stderr, "TX_VERIFY: p%d unknown card v%d s%d missing from pool (logs=%d)\n",
...
int torpex_strategy_choose(const Game *g, int bot_idx,
    ...
    if (!tx_flags_loaded) {
        tx_no_solve  = tx_flag("TX_NO_SOLVE");
        tx_no_voids  = tx_flag("TX_NO_VOIDS");
        tx_no_flip   = tx_flag("TX_NO_FLIP");
        tx_no_floors = tx_flag("TX_NO_FLOORS");
        ...
        // actual opponents beats assuming perfect play. TX_LEAF=1 re-enables.
        tx_no_leaf   = !tx_flag("TX_LEAF");
        tx_no_avoid  = tx_flag("TX_NO_AVOID");
        tx_verify    = tx_flag("TX_VERIFY");
        tx_w1_override = tx_env_int("TX_W1", 0);
        tx_w2_override = tx_env_int("TX_W2", 0);
        tx_w3_override = tx_env_int("TX_W3", -1);
        tx_keep1 = tx_env_int("TX_KEEP1", 0);
        tx_keep2 = tx_env_int("TX_KEEP2", 0);
        tx_rollout_policy = tx_env_int("TX_ROLLOUT", 0);
        tx_bb_win_budget = tx_env_int("TX_BB_WIN", 150000);
        tx_solve_cards = tx_env_int("TX_SOLVE_CARDS", 24);
        tx_bb_avoid_budget = tx_env_int("TX_BB_AVOID", 100000);
        tx_leaf_budget = tx_env_int("TX_LEAF_BUDGET", 1500);
        tx_leaf_max_cards = tx_env_int("TX_LEAF_CARDS", 10);
        tx_floor_mod = tx_env_int("TX_FLOOR_MOD", 2);
        if (tx_floor_mod < 1) tx_floor_mod = 1;
        tx_full_logs = tx_flag("TX_FULL_LOGS");
        tx_no_earlyexit = tx_flag("TX_NO_EARLYEXIT");
        tx_no_fastroll = tx_flag("TX_NO_FASTROLL");
        tx_bbleaf = tx_env_int("TX_BBLEAF", 2);
        tx_adapt = tx_env_int("TX_ADAPT", 1);
        tx_value_on = tx_env_int("TX_VALUE", 1) && tx_value_ready();
        tx_void_mod = tx_env_int("TX_VOID_MOD", 4);
        if (tx_void_mod < 2) tx_void_mod = 2;
        tx_profile = tx_env_int("TX_PROFILE", 0);
        tx_bbleaf_cards = tx_env_int("TX_BBLEAF_CARDS", 0);
        tx_bbleaf_cards2 = tx_env_int("TX_BBLEAF_CARDS2", 8);
        tx_bbleaf_budget = tx_env_int("TX_BBLEAF_BUDGET", 3000);
        tx_difftest = tx_flag("TX_DIFFTEST");
        if (tx_difftest) { void tx_difftest_report(void); atexit(tx_difftest_report); }
        tx_flags_loaded = 1;
    }
    ...
    Belief B;
    tx_build_belief(g, bot_idx, &B);
    if (tx_no_voids) for (int p = 0; p < MAX_PLAYERS; p++) B.void_n[p] = 0;
    if (tx_verify) tx_verify_belief(g, bot_idx, &B);

    // Player-count gate for the leaf lever (see tx_bbleaf comment). In the
    // pc-aware default mode heads-up uses the small-leaf threshold.
    tx_bbleaf_on = (tx_bbleaf != 0);
    tx_bbleaf_cards_eff = (tx_bbleaf == 2 && g->num_players == 2)
                        ? tx_bbleaf_cards2 : tx_bbleaf_cards;
    ...
    if (tx_profile) {
        bool any = false;
        for (int p = 0; p < g->num_players; p++) {
            tx_polmap_buf[p] = B.loose[p] ? CD_POL_LOOSE : CD_POL_HW;
            if (B.loose[p]) any = true;
        }
        if (any) tx_polmap = tx_polmap_buf;
    }
    ...
    int n_safe = moves->n;
    int solved = tx_no_solve ? -1
               : tx_try_endgame_solve(g, bot_idx, moves, &B, forced_loss, &n_safe);
    if (solved >= 0) {
    ...
    Candidates C;
    tx_pick_candidates(g, moves, forced_loss, &C);
    if (C.n == 0) {
    ...
    int W1, W2, W3;
    tx_params(g->num_players, &W1, &W2, &W3);

    uint32_t base = tx_mix((uint32_t)g->num_logs * 2654435761u,
                           ((uint32_t)g->deck_count << 8)
                           ^ (uint32_t)g->discard_pile_length
                           ^ ((uint32_t)bot_idx << 20));

    double score[TX_MAX_CANDS] = {0};
    int    nsim [TX_MAX_CANDS] = {0};
    bool   alive[TX_MAX_CANDS];
    ...
    // bitboards, and plays out. The struct path (TX_NO_FASTROLL / TX_LEAF /
    // TX_DIFFTEST) keeps the per-candidate Game clone for the leaf solver and
    // the exact-equivalence difftest.
    bool fast_path = !tx_no_fastroll && tx_no_leaf && !tx_difftest && !tx_flag("TX_NO_WORLDSIM");
    ...
    for (int stage = 0; stage < 3; stage++) {
        ...
        for (int w = w_lo; w < w_hi; w++) {
            uint32_t wseed = tx_mix(base, (uint32_t)(w + 1) * 0x85EBCA77u);
            ...
            bool use_voids  = (w % tx_void_mod) != tx_void_mod - 1;
            bool use_floors = !tx_no_floors && (w % tx_floor_mod) == 0;
            tx_sample_world(&world, g, bot_idx, &B, wseed, use_voids, use_floors);
            uint32_t sim_rng = tx_mix(wseed, 0x51AB1E5u);
            ...
            if (fast_path) {
                ...
                for (int ci = 0; ci < C.n; ci++) {
                    ...
                    if (!cd_sim_apply_root_move(&trial_sim, bot_idx,
                        ...
                        fpd = (double)g->num_players;
                    } else if (tx_value_on) {
                        // Learned value: predicted normalized finish mapped
                        // back to the 1..N position scale the pruning and
                        // cheap-first tie-break logic already speak.
                        fpd = 1.0 + (double)tx_value(&trial_sim, bot_idx)
                                  * (double)(g->num_players - 1);
                    } else {
                        int fp = (tx_bbleaf_on || tx_polmap)
                           ? cd_sim_playout_pol(&trial_sim, bot_idx, 600,
                                                !tx_no_earlyexit,
                                                tx_bbleaf_on ? tx_bbleaf_cards_eff : 0,
                                                tx_bbleaf_budget, tx_polmap)
                           : cd_sim_playout(&trial_sim, bot_idx, 600, !tx_no_earlyexit);
            ...
            for (int ci = 0; ci < C.n; ci++) {
                if (!alive[ci]) continue;
                tx_lite_clone(&trial, &world);
                game_rng_set(sim_rng);   // identical stream for every move
                if (!tx_apply(&trial, bot_idx, &moves->moves[C.idx[ci]])) {
                    score[ci] += (double)g->num_players;
                    nsim[ci]++;
                    continue;
                }
                int fp = tx_rollout(&trial, bot_idx, 600);
                if (fp == 0) fp = g->num_players;
                score[ci] += (double)fp;
                nsim[ci]++;
            }
        ...
        if (stage < 2) {
            ...
            int keep;
            if (stage == 0) {
                keep = (tx_keep1 > 0) ? tx_keep1 : (tx_oracle ? (C.n + 1) / 2 : C.n / 3);
                if (keep < (tx_oracle ? 4 : 3)) keep = tx_oracle ? 4 : 3;
            } else {
                keep = (tx_keep2 > 0) ? tx_keep2 : (tx_oracle ? 3 : 2);
            }
...
int torpex_oracle_strategy_choose(const Game *g, int bot_idx,
                                  const LegalMoves *moves, void *ctx) {
    tx_oracle = 1;
    int r = torpex_strategy_choose(g, bot_idx, moves, ctx);
    tx_oracle = 0;
    return r;
}
