...
function readVars() {
  ...
  ["--ground", "--ink", "--ink-2", "--ink-3", "--panel", "--rule", "--edge",
   "--edge-cross", "--edge-hard", "--warn", "--accent"]
...
/* --------------------------------------------------------- layered graph */
// A drawn hierarchy rather than an indented list: ranks running downward, one
// row per call depth, links between them. Call graphs are not trees - a rank
// assignment by BFS depth means any edge landing on an equal or shallower rank
// is a BACK EDGE, i.e. a cycle, and those are drawn dashed rather than dropped.
// Ranks run LEFT TO RIGHT, not top to bottom. Function names are wide and a
...
function textWidth(str) {
  if (!measurer) {
    const svg = svgEl("svg", { width: 0, height: 0,
                               style: "position:absolute;visibility:hidden" });
    measurer = svgEl("text");
    measurer.setAttribute("font-family", "'IBM Plex Mono', ui-monospace, monospace");
    measurer.setAttribute("font-size", "11");
    svg.append(measurer);
...
function buildLayered(root, dir, maxDepth) {
  const rank = new Map([[root, 0]]);
  const order = [root];
  let cut = false;
  for (let qi = 0; qi < order.length; qi++) {
    const u = order[qi], d = rank.get(u);
    if (d >= maxDepth) continue;
    for (const [v] of edgesOf(u)) {
      if (rank.has(v)) continue;
      if (order.length >= MAX_NODES) { cut = true; break; }
      rank.set(v, d + 1);
      order.push(v);
    }
    if (cut) break;
  ...
  // Rank by BFS depth, so every edge that lands on an equal or shallower rank
  // is a back edge - a cycle. Those are kept and drawn dashed, never dropped.
  const links = [];
  let back = 0;
  for (const u of order) {
    for (const [v, c] of edgesOf(u)) {
      if (!rank.has(v)) continue;
      const isBack = rank.get(v) <= rank.get(u);
      if (isBack) back++;
      links.push({ u, v, conf: G.confs[c], back: isBack });
    }
  }
  const layers = [];
  for (const [n, r] of rank) (layers[r] || (layers[r] = [])).push(n);

  const up = new Map(), down = new Map();
  for (const l of links) {
    if (l.back) continue;              // a cycle is what we route around, not
    if (!up.has(l.v)) up.set(l.v, []); // a constraint on the ordering
    if (!down.has(l.u)) down.set(l.u, []);
    up.get(l.v).push(l.u);
    down.get(l.u).push(l.v);
  }

  // Crossing reduction: order each rank by the median position of its
  // neighbours in the rank before (then after), sweeping both ways.
  const pos = new Map();
  layers.forEach(L => L && L.forEach((n, i) => pos.set(n, i)));
  const median = (n, side) => {
    const nb = (side === "up" ? up : down).get(n);
    if (!nb || !nb.length) return pos.get(n);
    const xs = nb.map(m => pos.get(m)).sort((a, b) => a - b);
    return xs[(xs.length - 1) >> 1];
  };
  for (let sweep = 0; sweep < 8; sweep++) {
    const side = sweep % 2 ? "down" : "up";
    const rs = layers.map((_, i) => i).filter(i => layers[i]);
    for (const r of (side === "up" ? rs : [...rs].reverse())) {
      layers[r].sort((a, b) => median(a, side) - median(b, side));
      layers[r].forEach((n, i) => pos.set(n, i));
    }
  }

  // Down the column: start on the index, then pull each node toward the median
  // of its neighbours so a chain runs straight instead of stair-stepping, and
  // re-separate after every pull so two boxes can never overlap.
  const STEP = NODE_H + ROW_GAP;
  const y = new Map();
  for (const L of layers) if (L) L.forEach((n, i) => y.set(n, i * STEP));
  for (let pass = 0; pass < 10; pass++) {
    for (const L of layers) {
      if (!L) continue;
      for (const n of L) {
        const nb = [...(up.get(n) || []), ...(down.get(n) || [])];
        if (!nb.length) continue;
        const ys = nb.map(m => y.get(m)).sort((a, b) => a - b);
      ...
      for (let i = L.length - 2; i >= 0; i--)
        if (y.get(L[i + 1]) - y.get(L[i]) < STEP) y.set(L[i], y.get(L[i + 1]) - STEP);
    }
  }

  // Columns are as wide as their widest name.
  const w = new Map();
  for (const n of order) w.set(n, Math.max(40, textWidth(clipName(G.name[n])) + 20));
  const colX = [], colW = [];
  let cx = 16;
  for (let r = 0; r < layers.length; r++) {
    colW[r] = layers[r] ? Math.max(...layers[r].map(n => w.get(n))) : 0;
    colX[r] = cx;
    cx += colW[r] + COL_GAP;
  }
  ...
  for (const n of order) {
    miny = Math.min(miny, y.get(n));
    maxy = Math.max(maxy, y.get(n));
  }
  const TOP = 34;                       // room for the rank captions
  for (const n of order) y.set(n, y.get(n) - miny + TOP + NODE_H / 2);

  return { order, rank, layers, links, y, w, colX, colW, back, cut,
           width: cx - COL_GAP + 16,
...
function renderHier(frag) {
  ...
  if (tree.root < 0) {
    ...
    const p = document.createElement("div");
    p.className = "treehint";
    p.innerHTML = "<b>Pick a function first.</b> The layered graph draws " +
      "everything reachable from one function, a column per call step.";
    frag.append(p);
    return;
  }
  const L = buildLayered(tree.root, tree.dir, tree.depth);
  ...
  for (const [id, col] of [["ah", CSSV["--edge-hard"]], ["ahb", CSSV["--warn"]],
                           ["ahc", CSSV["--accent"]]]) {
    const m = svgEl("marker", { id, viewBox: "0 0 8 8", refX: 7, refY: 4,
                                markerWidth: 6, markerHeight: 6, orient: "auto" });
    m.append(svgEl("path", { d: "M0 0 L8 4 L0 8 z", fill: col }));
    defs.append(m);
  }
  svg.append(defs);

  const left = n => L.colX[L.rank.get(n)];
  const right = n => L.colX[L.rank.get(n)] + L.w.get(n);

  for (let r = 0; r < L.layers.length; r++) {
    if (!L.layers[r]) continue;
    const t = svgEl("text", { class: "rank", x: L.colX[r], y: 18 });
    t.textContent = r === 0 ? "root"
      : r + (tree.dir === "out" ? " step out" : " step up");
    svg.append(t);
  }
  ...
  for (const l of L.links) {
    ...
    let d, cls = "lnk";
    if (l.back) {
      // A cycle returns leftward: bow it below the boxes so it reads as a
      // return path instead of hiding under the forward links.
      const x1 = left(l.u), x2 = right(l.v);
      const dip = 22 + Math.min(60, Math.abs(y2 - y1) * 0.3);
      d = "M" + x1 + " " + y1 + " C" + (x1 - dip * 1.6) + " " + (y1 + dip) + " " +
          (x2 + dip * 1.6) + " " + (y2 + dip) + " " + x2 + " " + y2;
      cls += " back";
    } else {
      const x1 = right(l.u), x2 = left(l.v), mx = (x1 + x2) / 2;
      d = "M" + x1 + " " + y1 + " C" + mx + " " + y1 + " " + mx + " " + y2 +
          " " + x2 + " " + y2;
      if (l.conf === "cross") cls += " cross";
    }
    const p = svgEl("path", { class: cls, d });
    p.setAttribute("marker-end", l.back ? "url(#ahb)"
      : l.conf === "cross" ? "url(#ahc)" : "url(#ah)");
  ...
  for (const n of L.order) {
    ...
    g.append(svgEl("rect", { x, y, width: L.w.get(n), height: NODE_H, rx: 3,
                             ...
                             stroke: COLOR[n] }));
    const t = svgEl("text", { x: x + 10, y: L.y.get(n) });
    t.textContent = clipName(G.name[n]);
    g.append(t);
    const title = svgEl("title");
    title.textContent = G.name[n] + "\n" + G.files[G.file[n]] + ":" + G.line[n];
    g.append(title);
  ...
  wrap.addEventListener("pointerdown", ev => {
    ...
    dragging = true; sx0 = ev.clientX; sy0 = ev.clientY;
    l0 = wrap.scrollLeft; t0 = wrap.scrollTop;
    wrap.setPointerCapture(ev.pointerId); wrap.classList.add("drag");
  });
  wrap.addEventListener("pointermove", ev => {
    if (!dragging) return;
    wrap.scrollLeft = l0 - (ev.clientX - sx0);
    wrap.scrollTop = t0 - (ev.clientY - sy0);
  });
...
function renderRootBar() {
  ...
  if (tree.kind === "struct") { bar.hidden = true; return; }
  bar.hidden = false;
  bar.innerHTML = "";
...
function renderTree() {
  ...
  tree.rows = 0;
  // The graph pane manages its own scrolling box; the list panes scroll as a
  // plain document.
  treeBody.classList.toggle("graphmode", tree.kind === "graph");
  ...
  const way = tree.dir === "out" ? "calls out" : "callers";
  document.getElementById("fmode").textContent =
    tree.kind === "struct" ? "directory tree"
      : tree.kind === "graph" ? "layered graph · " + way + " · depth " + tree.depth
        : "call tree · " + way;
}
...
function setTreeKind(kind) {
  tree.kind = kind;
  if (kind !== "struct" && st.sel >= 0) tree.root = st.sel;
  ...
  treeBar.dirSeg.hidden = kind === "struct";
