...
function buildLayered(root) {
  ...
  for (const u of order) {
    for (const [v, c] of edgesOf(u, "out")) {
      if (!rank.has(v)) continue;
      const k = u + ">" + v;
  ...
  // Columns are the ranks present, left to right.
  const ranks = [...new Set(rank.values())].sort((a, b) => a - b);
  const col = new Map(ranks.map((r, i) => [r, i]));
  const layers = ranks.map(() => []);
  for (const [n, r] of rank) layers[col.get(r)].push(n);

  const up = new Map(), down = new Map();
  for (const l of links) {
    if (l.back) continue;
    if (!up.has(l.v)) up.set(l.v, []);
  ...
  const pos = new Map();
  layers.forEach(L => L.forEach((n, i) => pos.set(n, i)));
  const median = (n, side) => {
  ...
  const y = new Map();
  for (const L of layers) L.forEach((n, i) => y.set(n, i * STEP));
  ...
  for (let c = 0; c < layers.length; c++) {
    colW[c] = layers[c].length ? Math.max(...layers[c].map(n => w.get(n))) : 0;
    colX[c] = cx;
    cx += colW[c] + COL_GAP;
  }
