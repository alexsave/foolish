...
"$DG" "${t[@]}" --table DG_GRID --labels DG_GRID.0=DG_ROW_NAME --labels DG_GRID.1=DG_COL_NAME --ts "$tmp/a.ts" --swift "$tmp/a.swift"
"$DG" "${t[@]}" --table DG_GRID --labels DG_GRID.0=DG_ROW_NAME --labels DG_GRID.1=DG_COL_NAME --ts "$tmp/b.ts" --swift "$tmp/b.swift"
cmp -s "$tmp/a.ts" "$tmp/b.ts" && cmp -s "$tmp/a.swift" "$tmp/b.swift"
...
ok $? "both modules carry a banner naming the C table they came from"
...
ok $? "the row type carries each column's own C type into both languages"
