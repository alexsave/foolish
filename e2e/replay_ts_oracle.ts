/* =============================================================================
 * FROZEN v5 replay-codec reference (TEST-ONLY oracle)
 * =============================================================================
 * This is the original TypeScript implementation of the replay wire format,
 * verbatim, kept ONLY as a differential-testing oracle. Production encode/
 * decode run in the C kernel (cnitro/src/replay.c, via _shared/wasm/
 * engine.ts); e2e/replay_codec.test.ts asserts the kernel produces
 * byte-identical encodings and identical decodes to this file on real
 * engine-played games.
 *
 * DO NOT import from production code, and DO NOT "fix" or modernize this
 * file: its value is that it is the frozen v5 spec. If the kernel and this
 * file disagree, the difftest fails and a human decides which side is wrong.
 * ========================================================================== */

import { Card, LOG_TYPE, LogType, LogCardPair } from "../supabase/functions/_shared/types.ts";
import { ACE_VALUE, CARDS_PER_PLAYER } from "../supabase/functions/_shared/constants.ts";
import { canCover } from "../supabase/functions/_shared/common_utils.ts";
import { Coder, comb, bigintToBytes, base32Encode, base64Encode, gameToUrl } from "../supabase/functions/_shared/replay/codec.ts";
...
export function oracleEncodeReplay(input: ReplayInput): EncodedReplay {
...
/* ===================== DECODE entry (frozen, verbatim) ==================== */

export function oracleDecodeReplay(x: bigint): DecodedReplay {
